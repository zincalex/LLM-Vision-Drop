...
def load_model_for_healing(dropped_model_path: str, original_model_path: str, accelerator: Accelerator):
    ...
    config_kwargs = {
        ...
        "cache_dir": None, "revision": "main", "token": None,
    ...
    model = auto_cls.from_pretrained(
        original_model_path, torch_dtype=torch.bfloat16, **config_kwargs)
...
def save_healed_model(model, processor, output_dir: str, accelerator: Accelerator):
    ...
    if is_peft:
        accelerator.print("Saving LoRA adapters...")
        unwrapped_model.save_pretrained(output_dir, safe_serialization=True)
    else:
        accelerator.print("Saving model directly...")
