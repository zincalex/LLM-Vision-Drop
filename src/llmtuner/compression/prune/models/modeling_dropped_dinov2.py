...
from .configuration_dropped_dinov2 import Dinov2Config
...
class Dinov2Layer(GradientCheckpointingLayer):
    ...
    def __init__(self, config: Dinov2Config, layer_idx: int) -> None:
        super().__init__()
    ...
    def forward(
        ...
        if not self.drop_mlp:
            ...
            hidden_states = self.drop_path(layer_output) + hidden_states
...
class Dinov2Encoder(nn.Module):
    def __init__(self, config: Dinov2Config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([Dinov2Layer(config, layer_idx=i) for i in range(config.num_hidden_layers)])
