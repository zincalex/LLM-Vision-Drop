...
from .configuration_dropped_swinv2 import Swinv2Config
...
class Swinv2Layer(nn.Module):
    def __init__(
        self, config, dim, input_resolution, num_heads, drop_path_rate=0.0, shift_size=0, pretrained_window_size=0, layer_idx=0
    ):
        super().__init__()
        ...
        # ADD dropping logic for attention
    ...
    def forward(
        ...
        layer_outputs = (layer_output, attention_outputs[1]) if output_attentions and not self.drop_attn else (layer_output,)
        return layer_outputs


class Swinv2Stage(GradientCheckpointingLayer):
    def __init__(
        self, config, dim, input_resolution, depth, num_heads, drop_path, downsample, pretrained_window_size=0, start_layer_idx=0
    ):
        super().__init__()
        self.config = config
        self.dim = dim
    ...
    def __init__(self, config, grid_size, pretrained_window_sizes=(0, 0, 0, 0)):
        ...
        for i_layer in range(self.num_layers):
            stage = Swinv2Stage(
                ...
                start_layer_idx=layer_idx_offset,
            )
            layers.append(stage)
            layer_idx_offset += config.depths[i_layer]
